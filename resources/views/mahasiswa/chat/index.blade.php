        ...
        <div class="hidden lg:block w-64 flex-shrink-0">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden sticky top-4">
                ...
                <div class="max-h-96 overflow-y-auto">
                    @foreach($classes as $c)
                        <a href="{{ route('mahasiswa.chat.class', $c) }}" class="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 transition-colors {{ $class->id === $c->id ? 'bg-blue-50 border-l-2 border-l-blue-500' : '' }}">
    ...
    <div class="flex gap-6">
        <div class="hidden lg:block w-64 flex-shrink-0">
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden sticky top-4">
                ...
                <div class="max-h-96 overflow-y-auto">
                    @foreach($classes as $c)
                        <a href="{{ route('mahasiswa.chat.class', $c) }}" class="flex items-center gap-3 px-3 py-2.5 hover:bg-gray-50 transition-colors">
                            <div class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">{{ substr($c->name, 0, 2) }}</div>
                            <div class="min-w-0">
                                <p class="text-sm font-medium text-gray-800 truncate">{{ $c->name }}</p>
                                <p class="text-xs text-gray-400">Klik untuk chat</p>
