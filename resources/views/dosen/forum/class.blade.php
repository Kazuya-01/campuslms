    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5">
        <div class="flex items-center justify-between mb-4">
            ...
            <div class="flex items-center gap-2">
                <a href="{{ route('dosen.forum.index') }}" class="text-sm text-gray-500 dark:text-gray-400 hover:text-emerald-600">← Semua Forum</a>
                <a href="{{ route('dosen.forum.create', $class) }}" class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white text-sm font-medium rounded-lg transition-all shadow-sm">
                    + Thread Baru
                </a>
            </div>
        </div>
        ...
        <div class="space-y-1">
            @forelse($class->forumThreads as $thread)
                <a href="{{ route('dosen.forum.show', $thread) }}" class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
            ...
            @empty
                <p class="text-gray-500 dark:text-gray-400 text-sm py-4 text-center">Belum ada thread diskusi. Buat thread baru untuk memulai diskusi.</p>
            @endforelse
        </div>
    </div>
</div>
