...
<script>
    ...
    function appendMessage(msg, isSelf) {
        ...
        d.innerHTML = `<div class="max-w-[85%] lg:max-w-[70%] ${isSelf ? 'bg-emerald-500 text-white rounded-2xl rounded-br-md' : 'bg-gray-100 text-gray-800 rounded-2xl rounded-bl-md'} px-4 py-2.5">${!isSelf ? `<p class="text-xs font-medium text-blue-600 mb-0.5">${msg.user?.name || ''}</p>` : ''}<p class="text-sm leading-relaxed break-words">${escHtml(msg.message)}</p><p class="text-xs mt-1 ${isSelf ? 'text-emerald-200' : 'text-gray-400'}">${new Date().toLocaleTimeString('id-ID', {hour:'2-digit',minute:'2-digit'})}</p></div>`;
